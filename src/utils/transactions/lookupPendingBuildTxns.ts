...
export default async function lookupPendingBuildTxns(
  chain: ChainType,
  block: number,
  managerAddr: string,
  ...
  try {
    ...
    const encodedPrefix = Buffer.from(`awebuild_${managerAddr}`).toString(
      `base64`,
    );
    let response = await indexerForChain(chain)
      .lookupAccountTransactions(managerAddr)
      .txType(`axfer`)
